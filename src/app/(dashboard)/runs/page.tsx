...
<head>
  <meta charset="UTF-8">
  <title>${run.brainId} — FlowOS Report</title>
  <style>
    body { 
    ...
    h1 { font-size: 2em; font-weight: 400; border-bottom: 2px solid #111; padding-bottom: 16px; }
    h2 { font-size: 1.4em; font-weight: 600; margin-top: 40px; color: #111; }
    h3 { font-size: 1.1em; font-weight: 600; color: #333; }
    p { margin: 12px 0; }
    ...
    .header { 
      ...
      padding: 24px 40px; margin: -60px -40px 40px;
    ...
    .meta { color: #999; font-size: 0.85em; margin-bottom: 40px; font-family: sans-serif; }
    .footer { margin-top: 60px; padding-top: 20px; border-top: 1px solid #eee; color: #999; font-size: 0.8em; font-family: sans-serif; }
  </style>
</head>
<body>
  <div class="header">
    <span style="font-size:24px">⚡</span>
    <h1>FlowOS Research Report</h1>
  </div>
  <div class="meta">Generated on ${new Date().toLocaleDateString("en-IN", { day: "numeric", month: "long", year: "numeric" })} · ${run.brainId}</div>
  ${cleanOutput
    .split("\n")
    .map((line) => {
      ...
      if (line.match(/^\d+\. /))
        return `<li>${line.replace(/^\d+\. /, "").replace(/\*\*(.*?)\*\*/g, "<strong>$1</strong>")}</li>`
      if (line.trim() === "") return "<br>"
      return `<p>${line.replace(/\*\*(.*?)\*\*/g, "<strong>$1</strong>")}</p>`
    })
    .join("\n")}
  <div class="footer">Generated by FlowOS Jarvis · flowos.in</div>
</body>
</html>`
